...
// app.use(cors({
//   origin: 'http://localhost:3300', // Your Next.js app URL
//   methods: ['GET', 'POST'],
//   credentials: true
// }));
    ...
    const interval = setInterval(async () => {
      try {
        const resultCount = await client.query(
          "SELECT count(*) as count FROM log_activity WHERE no_perkara IN (SELECT no_perkara FROM perkara WHERE tgl_selesai_perkara IS NULL)"
        );
        console.log("count: ", resultCount.rows[0].count);

        const totalRecords = parseInt(resultCount.rows[0].count, 10);

        // if (totalRecords !== dataCount) {
